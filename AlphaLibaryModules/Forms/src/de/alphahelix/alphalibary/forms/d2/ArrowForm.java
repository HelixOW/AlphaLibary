        ...
        if (getAxis().equalsIgnoreCase("x")) {
            if (direction == BlockFace.UP) {
                ...
                for (double x = ((width / 2) * (-1)); x < 0; x += getDense())
                    getAction().action(p, getLocation().clone().add(x, getFormFunctions()[1].f(x), 0));

                for (double x = 0; x < lenght; x += getDense())
                    getAction().action(p, getLocation().clone().subtract(0, getFormFunctions()[1].f(x), 0));
                ...
                for (double x = ((width / 2) * (-1)); x < 0; x += getDense())
                    getAction().action(p, getLocation().clone().subtract(x, getFormFunctions()[1].f(x), 0));

                for (double x = 0; x < lenght; x += getDense())
                    getAction().action(p, getLocation().clone().add(0, getFormFunctions()[1].f(x), 0));
            }
            ...
            if (direction == BlockFace.UP) {
                ...
                for (double z = ((width / 2) * (-1)); z < 0; z += getDense())
                    getAction().action(p, getLocation().clone().add(0, getFormFunctions()[1].f(z), z));

                for (double z = 0; z < lenght; z += getDense())
                    getAction().action(p, getLocation().clone().subtract(0, getFormFunctions()[1].f(z), 0));
            } else {
                for (double z = 0; z < (width / 2); z += getDense())
                    getAction().action(p, getLocation().clone().subtract(0, getFormFunctions()[0].f(z), z));

                for (double z = ((width / 2) * (-1)); z < 0; z += getDense())
                    getAction().action(p, getLocation().clone().subtract(0, getFormFunctions()[1].f(z), z));

                for (double z = 0; z < lenght; z += getDense())
                    getAction().action(p, getLocation().clone().add(0, getFormFunctions()[1].f(z), 0));
            }
            ...
            if (direction == BlockFace.UP) {
                ...
                for (double xz = ((width / 2) * (-1)); xz < 0; xz += getDense())
                    getAction().action(p, getLocation().clone().add(xz, getFormFunctions()[1].f(xz), xz));

                for (double xz = 0; xz < lenght; xz += getDense())
                    getAction().action(p, getLocation().clone().subtract(0, getFormFunctions()[1].f(xz), 0));
            } else {
                for (double xz = 0; xz < (width / 2); xz += getDense())
                    getAction().action(p, getLocation().clone().subtract(xz, getFormFunctions()[0].f(xz), xz));

                for (double xz = ((width / 2) * (-1)); xz < 0; xz += getDense())
                    getAction().action(p, getLocation().clone().subtract(xz, getFormFunctions()[1].f(xz), xz));

                for (double xz = 0; xz < lenght; xz += getDense())
                    getAction().action(p, getLocation().clone().add(0, getFormFunctions()[1].f(xz), 0));
            }
            ...
            if (direction == BlockFace.EAST) {
                ...
                for (double xy = ((width / 2) * (-1)); xy < 0; xy += getDense())
                    getAction().action(p, getLocation().clone().add(getFormFunctions()[1].x(xy), 0, xy));

                for (double xy = 0; xy < lenght; xy += getDense())
                    getAction().action(p, getLocation().clone().subtract(getFormFunctions()[1].x(xy), 0, 0));

            } else {
                for (double xy = 0; xy < (width / 2); xy += getDense())
                    getAction().action(p, getLocation().clone().subtract(getFormFunctions()[0].x(xy), 0, xy));

                for (double xy = ((width / 2) * (-1)); xy < 0; xy += getDense())
                    getAction().action(p, getLocation().clone().subtract(getFormFunctions()[1].x(xy), 0, xy));

                for (double xy = 0; xy < lenght; xy += getDense())
                    getAction().action(p, getLocation().clone().add(getFormFunctions()[1].x(xy), 0, 0));
            ...
            if (direction == BlockFace.SOUTH) {
                ...
                for (double zy = ((width / 2) * (-1)); zy < 0; zy += getDense())
                    getAction().action(p, getLocation().clone().add(zy, 0, getFormFunctions()[1].x(zy)));

                for (double zy = 0; zy < lenght; zy += getDense())
                    getAction().action(p, getLocation().clone().subtract(0, 0, getFormFunctions()[1].x(zy)));

            } else {
                for (double zy = 0; zy < (width / 2); zy += getDense())
                    getAction().action(p, getLocation().clone().subtract(zy, 0, getFormFunctions()[0].x(zy)));

                for (double zy = ((width / 2) * (-1)); zy < 0; zy += getDense())
                    getAction().action(p, getLocation().clone().subtract(zy, 0, getFormFunctions()[1].x(zy)));

                for (double zy = 0; zy < lenght; zy += getDense())
                    getAction().action(p, getLocation().clone().add(0, 0, getFormFunctions()[1].x(zy)));
            ...
            if (direction == BlockFace.SOUTH_EAST) {
                for (double xyz = 0; xyz < (width / 2); xyz += getDense())
                    getAction().action(p, getLocation().clone().add(getFormFunctions()[0].x(xyz), 0, getFormFunctions()[0].x(xyz)));

                for (double xyz = ((width / 2) * (-1)); xyz < 0; xyz += getDense())
                    getAction().action(p, getLocation().clone().add(getFormFunctions()[1].x(xyz), 0, getFormFunctions()[1].x(xyz)));

                for (double xyz = 0; xyz < lenght; xyz += getDense())
                    getAction().action(p, getLocation().clone().subtract(getFormFunctions()[1].x(xyz), 0, getFormFunctions()[1].x(xyz)));

            } else {
                for (double xyz = 0; xyz < (width / 2); xyz += getDense())
                    getAction().action(p, getLocation().clone().subtract(getFormFunctions()[0].x(xyz), 0, getFormFunctions()[0].x(xyz)));

                for (double xyz = ((width / 2) * (-1)); xyz < 0; xyz += getDense())
                    getAction().action(p, getLocation().clone().subtract(getFormFunctions()[1].x(xyz), 0, getFormFunctions()[1].x(xyz)));

                for (double xyz = 0; xyz < lenght; xyz += getDense())
                    getAction().action(p, getLocation().clone().add(xyz, 0, getFormFunctions()[1].x(xyz)));
            }
        }
    }
}
